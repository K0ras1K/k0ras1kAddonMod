...
public class BlockNeutronBig extends BlockContainer {
    ...
    public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
        if (side == 1)
            return top;
        int facing = 2;
        TileEntityNeutronBig machine = (TileEntityNeutronBig) world.getTileEntity(x, y, z);
        if (machine != null)
    ...
    public TileEntity createNewTileEntity(World world, int meta) {
        return new TileEntityNeutronBig();
    }

    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase player, ItemStack item) {
        TileEntity tile = world.getTileEntity(x, y, z);
        if (tile instanceof TileEntityNeutronBig) {
            TileEntityNeutronBig machine = (TileEntityNeutronBig) tile;
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int wut) {
        TileEntityNeutronBig collector = (TileEntityNeutronBig) world.getTileEntity(x, y, z);
